...
(function () {
  ...
  function handleMessage(request, sender, sendResponse) {
    ...
    switch (request.type) {
      ...
      case 'runCookieAutomation': {
        // Handle cookie automation requests
        try {
          const { cookies, validationSelector } = request.params;
          
          // Import the automation class
          import('./interface/lib/cookieAutomation.js').then(async ({ CookieAutomation }) => {
            const automation = new CookieAutomation(browserDetector, null);
            
            // Set cookies and validate domains
            const results = await automation.runAutomationWithCookies(cookies, validationSelector);
          ...
          console.error('Cookie automation failed:', error);
          sendResponse({ success: false, error: error.message });
        }
        return true;
      }
